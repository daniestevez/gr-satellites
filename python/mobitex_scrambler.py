...
class Scrambler:
    ...
    def scramble(self, bit):
        ...
        if ((self.scramble_shift_reg & 0x0011) == 0x0010) or ((self.scramble_shift_reg & 0x0011) == 0x0001):
...
if __name__ == '__main__':
    packet_out = list(bytes.fromhex(
        '00 00 20 00 01 00 00 00 00 0C 00 FF 00 F0 F3 33 33 25 55 57 0F F0 06 66 6A 33 CC C5 A5 A9'
    ))

    # Unpack bits
    packet_bits = []
    for byte in packet_out:
        for i in range(8):
            bit = (byte >> (7 - i)) & 0x01
            print(bit, end='')
        ...
        print(' ', end='')
    print('')
    print(f'{bytes(packet_out).hex(' ', 1).upper()}')

    for i in range(8):
        print(packet_bits[i * 8:i * 8 + 8])
