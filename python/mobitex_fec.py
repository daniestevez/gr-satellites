...
It is a (12,8,3) linear code, thus it can encode 8-bit data with 4-bit FEC codes and
detect/correct single-bit errors in the combined 12-bit words.

- Reliable two-bit error correction is not possible with this FEC scheme.
- Many collisions occur where a two-bit error produces the same syndrome as a single-bit error.

References:
[1]: https://git.tu-berlin.de/rft/com/mobitub-2/-/blob/master/gr-tnc_nx/
[2]: https://ocw.mit.edu/courses/6-02-introduction-to-eecs-ii-digital-communication-systems-fall-2012/1cc286051b660feded4d17650bd21d61_MIT6_02F12_chap05.pdf
...
# SYNDROME_TABLE = {1: 0, 2: 1, 4: 2, 8: 3, 5: 4, 6: 5, 9: 6, 10: 7, 7: 8, 11: 9, 13: 10, 14: 11}
...
def encode(message: int) -> tuple[int, int]:
    ...
    for i, mask in enumerate(FEC_MATRIX_8b):
        ...
        fec |= (parity << (3 - i))
        # print(f'{i} - A 0x{mask:02X} - B 0x{message:02X} | 0x{masked:03X} - {parity} - {fec:03x}')
...
def decode(codeword: int):
    """
    Takes 12-bit codeword, returns tuple of (corrected_message, corrected_fec, status)

    output:
        corrected_message (int): 1 control byte, error corrected
        corrected_fec (int)    : 4 parity bits, error corrected
        status (Status)        : (0 - NO ERROR, 1 - ERROR CORRECTED, 2 - ERROR UNCORRECTABLE)
    """
