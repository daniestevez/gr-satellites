...
encode_rs::sptr encode_rs::make(int basis)
{
    return gnuradio::get_initial_sptr(new encode_rs_impl(basis));
}
...
void encode_rs_impl::msg_handler(pmt::pmt_t pmt_msg)
{
    ...
    if (length > MAX_FRAME_LEN - PARITY_BYTES)
        return;
    ...
    if (d_basis == BASIS_CONVENTIONAL) {
        encode_rs_8(
            d_data.data(), &d_data[length], MAX_FRAME_LEN - length - PARITY_BYTES);
    } else {
        encode_rs_ccsds(
            d_data.data(), &d_data[length], MAX_FRAME_LEN - length - PARITY_BYTES);
    }
