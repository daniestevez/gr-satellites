                                                      ...
                                                      int num_filters)
{
    return gnuradio::get_initial_sptr(
        new time_dependent_delay_impl(filename, samp_rate, t0, taps, num_filters));
}
      ...
      d_current_delay(0.0)
{
    for (int nfilt = 0; nfilt < num_filters; ++nfilt) {
        ...
        d_filters.emplace_back(decimation, filt_taps);
    }
...
void time_dependent_delay_impl::read_delay_file(const std::string& filename)
{
    ...
    while (!input_file.eof()) {
        ...
        if (delay_samples < d_fir_delay) {
            throw std::runtime_error("delay is smaller than FIR group delay");
        }
...
void time_dependent_delay_impl::set_time(double t)
{
    ...
    d_t0 = t;
    printf("[time_dependent_delay] set time %f at sample %ld\n", d_t0, d_sample_t0);
