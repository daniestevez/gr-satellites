...
int main(int argc, char **argv) {
  ...
  std::vector<float> pulse = gr::filter::firdes::root_raised_cosine(n_poly * sps,
								    n_poly * sps,
								    1.0,
								    rrc_alpha,
								    ceil(11 * n_poly * sps));

  std::vector<float> taps = gr::filter::firdes::root_raised_cosine(n_filters,
								   n_filters,
								   1.0 / sps,
								   rrc_alpha,
								   ceil(11 * sps * n_filters));
  ...
  for (int ii = 0; ii < sweep_n_symbols * n_poly * sps ; ii++) {
    ...
    if (ii == 1) {
      std::cout << "Gain " << (ted_error_old - ted_error)*n_poly << " sample^{-1}" << std::endl;
    }
